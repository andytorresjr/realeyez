import { CandidatePost, ViewerSignals } from './types';

const HOUR_MS = 1000 * 60 * 60;

function recencyScore(createdAtMs: number, nowMs: number): number {
  const ageHours = Math.max((nowMs - createdAtMs) / HOUR_MS, 0);
  return Math.exp(-ageHours / 24);
}

function interestScore(hashtags: string[], likedHashtags: Set<string>): number {
...
function engagementScore(likes: number, comments: number, watchRatio: number): number {
...
export function rankForYouFeed(posts: CandidatePost[], viewer: ViewerSignals): RankedPost[] {
  return posts
